...
\tikzstyle{io} = [trapezium, trapezium left angle=70, trapezium right angle=110, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=tableau_blue_bg]
\tikzstyle{process} = [rectangle, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=tableau_orange_bg]
...
\begin{Figure}
    ...
    \begin{tikzpicture}[node distance=4cm, every text node part/.style={align=center}]
        ...
        \node (measure) [io, right of=start, xshift=2cm] {Measure $\alpha$};
        \draw [arrow] (start) -- (measure);

        \node (isaccelerating) [decision, right of=measure, xshift=2cm] {$\alpha \geq \alpha_{min}$};
        \draw [arrow] (measure) -- (isaccelerating);

        \node (ispushing) [decision, right of=isaccelerating, xshift=2cm] {pushing};
        \draw [arrow] (isaccelerating) -- node[anchor=south] {no} (ispushing);

        \node (pushing) [process, right of=ispushing, xshift=2cm] {pushing \\ not redlining};
        \draw [arrow] (ispushing) -- node[anchor=south] {no} (pushing);

        \node (isredlining) [decision, below of=ispushing, yshift=-1.5cm] {$a + d \geq a_{cap}$};
        \draw [arrow] (ispushing) -- node[anchor=west] {yes} (isredlining);

        \node (redlining) [process, right of=isredlining, xshift=2cm] {redlining};
        \draw [arrow] (isredlining) -- node[anchor=south] {yes} (redlining);

        \node (ispushing2) [decision, below of=isaccelerating, yshift=-1.5cm] {pushing};
        \draw [arrow] (isaccelerating) -- node[anchor=west] {yes} (ispushing2);
    \end{tikzpicture}
